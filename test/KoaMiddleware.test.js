...
import trackerMiddleware from '../lib/adapters/Koa.js';
...
describe('Koa tracker middleware', () => {
    let originalFetch;
    let fetchSpy;
    ...
    const createCtxRes = () => {
        ...
        return { ctx, res, finishListeners };
    };

    it('should attach Event instance to ctx.state.tracker and call next()', async () => {
        const middleware = trackerMiddleware({
            url: 'https://example.test',
            key: 'api-key'
        });

        const { ctx } = createCtxRes();
        const nextSpy = spy();

        await middleware(ctx, nextSpy);

        assert.equal(nextSpy.calls.length, 1);
        assert.ok(ctx.state.tracker);
        assert.equal(typeof ctx.state.tracker.getUuid, 'function');
    });
